...
def load_env_config():
    ...
    if os.environ['CHAIN']:
        ...
        tp.update({'is_boostrap_needed': os.environ['IS_BOOTSTRAP_NEEDED']})
...
def load_ac_params(asset, chain_mode='default'):
    if os.path.isfile('./chainconfig.json'):
        ...
        if chain_mode == 'REGTEST':
            ac.update({'daemon_params': " -daemon -regtest -whitelist=127.0.0.1"})
        else:
...
def main():
    ...
    for i in range(clients_to_start):  # start daemons
        cl_args = [ac_params.get['binary_path'],
                   ...
                   '-pubkey=' + ac_params.get('test_pubkey')[i],
                   ac_params.get('daemon_params')
                   ]
        if ac_params.get['ac_rewards']:
            clients_to_start.append('-ac_rewards=' + ac_params.get('ac_rewards'))
        else:
            clients_to_start.append('-ac_rewards=100000000000')
        if ac_params.get['ac_supply']:
            clients_to_start.append('-ac_supply=' + ac_params.get('ac_supply'))
        else:
            clients_to_start.append('-ac_supply=100000000000')
        if ac_params.get['ac_cc']:
            clients_to_start.append('-ac_cc=' + ac_params.get('ac_cc'))
        else:
            clients_to_start.append('-ac_cc=2')
